...
class LightPatternFlame(LightPattern):


    def __init__(self):
        super().__init__() 
        self.flamePixelOffset = 2
        self.flamePixelSize = 4
        self.time = 0
    ...
    def setPixelColors(self, color):
        ...
        for i in range(0, self.flamePixelOffset):
            self.lightSegment.setPixelColor(i, SIDE_PIXEL_COLOR)

        # Set RIGHT side pixels of display flame to solid color
        for i in range(self.flamePixelOffset +  self.flamePixelSize, self.lightSegment.numOfSegmentPixels):
            self.lightSegment.setPixelColor(i,SIDE_PIXEL_COLOR)

        # Set the FLAME pixels
        for i in range(self.flamePixelOffset, self.flamePixelOffset +  self.flamePixelSize):
